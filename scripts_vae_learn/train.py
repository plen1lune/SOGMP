...
def train(model, dataloader, dataset, device, optimizer, criterion, epoch, epochs):
    ...
    #for i, batch in enumerate(dataloader, 0):
        ...
        # llc: scan size is 128,20,1080, position size is 128,20,3, velocity size is 128,20,2
        # it means that the batch size is 128, time length is 20
        # 1080 is the number of lidar scan points, 3 is the x,y,theta of the robot position, 2 is the x,y of the robot velocity
        scans = batch['scan']
        ...
        # llc: shape is 128,10,64,64, 128 is the batch size, 10 is the time length, 64,64 is the map size
        ...
        # llc: shape is 128,3, 128 is the batch size, 3 is the x,y,theta of the robot position
        # robot positions:
        pos = positions[:,:SEQ_LEN]
        print('obs_pos_N:', obs_pos_N[0])
        print('pos:', pos[0])
